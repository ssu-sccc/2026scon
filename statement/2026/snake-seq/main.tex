\def\probtitle{뱀 수열}
\def\probno{L} % 문제 번호
\begin{problem}{\probtitle}{표준 입력(stdin)}{표준 출력(stdout)}{3 초}{1024 megabytes}
...
각 점의 좌표를 $P_i = (x_i,\, y_i)$로 쓴다.

뱀은 \textbf{오른쪽 위}로만 전진한다.
즉, 모든 $0 \leq i < L$에 대해 $x_i < x_{i+1}$이고 $y_i < y_{i+1}$이다.

뱀은 \textbf{지그재그}로 나아간다.
연속한 두 점 사이의 이동 기울기를 $s_i = \dfrac{y_{i+1} - y_i}{x_{i+1} - x_i}$ ($0 \leq i < L$)라 정의하자.
...
\begin{center}
    \includegraphics[width=0.75\linewidth]{image/img.png}\\
...
\begin{itemize}[topsep=0pt,noitemsep]
    ...
    \item $1 \le x, y \le 10^9$
    \item 같은 좌표에 놓인 사과는 없다.
\end{itemize}
